...
const OTPModal = ({ isOpen, onClose, onVerified, purpose = 'voting' }) => {
  ...
  const handleRequestOTP = async () => {
    setRequesting(true);
    try {
      const endpoint = purpose === 'voting' ? '/auth/request-vote-otp' : '/auth/login'; // Adjust based on purpose
      await API.post(endpoint);
      toast.success(`OTP sent to your registered email for ${purpose}.`);
    } catch (error) {
  ...
  const handleVerify = async (e) => {
    ...
    setLoading(true);
    try {
      const endpoint = purpose === 'voting' ? '/auth/verify-vote-otp' : '/auth/verify-otp';
      ...
      >
        <div className="p-6">
          ...
          <p className="mb-6 text-sm opacity-80" style={{ color: 'var(--text-color)' }}>
            To securely {purpose === 'voting' ? 'cast your vote' : 'complete this action'}, please enter the 6-digit code sent to your email.
          </p>
          ...
          <form onSubmit={handleVerify} className="space-y-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button 
                ...
                disabled={loading || requesting}
                className="text-sm font-medium hover:underline opacity-70 hover:opacity-100"
                style={{ color: 'var(--text-color)' }}
              >
